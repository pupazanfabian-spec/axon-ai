...
// Axon Code Generator — detectare tip app, system prompt expert, generare cod structurat
...
function buildExpertSystemPrompt(userRequest: string, projectContext?: string): string {
  ...
  let systemPrompt = `Ești Axon, un expert senior în dezvoltare software cu 15+ ani experiență.
...
function buildDebugPrompt(userRequest: string, codeSnippet?: string): string {
  return `Ești Axon, expert senior în debugging. Analizezi erori și oferi soluții precise.
Răspunzi în română.
...
export function buildAICodePrompt(
  ...
  switch (intent) {
    ...
    case 'explain':
    case 'compare':
      return `Ești Axon, expert senior în dezvoltare software. Explici concepte tehnice clar, în română, cu exemple practice.\n\nÎntrebare: ${userRequest}`;
  }
}
