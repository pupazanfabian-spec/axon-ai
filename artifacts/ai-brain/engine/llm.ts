...
export async function loadModel(): Promise<boolean> {
  ...
  try {
    ...
    console.warn('[Axon LLM] Model load failed:', e);
    _isLoading = false;
    return false;
  }
}
...
// ─── Prompt de sistem — Constituția + personalitatea Axon ────────────────────
...
function buildSystemPrompt(
  ...
  return `Ești Axon, un asistent AI personal care rulează 100% offline pe dispozitivul utilizatorului.
...
export async function generateLLMResponse(
  ...
  try {
    ...
    console.warn('[Axon LLM] Generation failed:', e);
    return null;
  }
}
